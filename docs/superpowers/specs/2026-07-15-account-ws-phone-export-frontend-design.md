...
实施阶段遵循项目现有测试方式，但只设计以下 5 个聚焦主流程的测试，不为每种状态值、文件名字符或响应异常分别扩展大量排列组合：

1. **菜单展示**：菜单项位于“一键抢登”之后、“批量删除”之前；未选择和导出中均禁用。
2. **非正常账号阻断**：混合勾选时展示正常与非正常数量，关闭弹窗后没有调用导出 API。
3. **正常账号确认**：全部正常时展示预计数量；单一分组显示分组文件名，多分组显示 `全部WS号_YYYY-MM-DD.txt`，确认后请求携带正确的 `ids/groupName`。
4. **导出成功**：POST Blob 响应中的 `Content-Disposition` 和 `X-Export-Count` 被正确解析，只触发一次下载，并使用后端实际数量展示成功提示。
5. **后端业务失败**：JSON Blob 的后端 `message` 被展示；“无有效WS号码”和通用导出失败均不生成文件。

纯逻辑、API 与页面编排可以按现有文件边界拆开实现测试，但总体验收以以上 5 条主流程为准。缺失 ID、用户取消和防重复提交继续在生产代码中防御，不单独扩展测试矩阵。
