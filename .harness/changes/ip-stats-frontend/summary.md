...
- In scope:
  ...
  - 展示 `覆盖国家数 = 有 IP 真实国家 / 支持国家` 与 `无 IP 国家数`。
- Out of scope:
  - 检测、抽检执行、全量检测。
  - 导出、批量删除。
...
- 总览 IP 数量包含混合池；国家维度统计、覆盖国家数和无 IP 国家数只统计真实支持国家，不包含 `混合（不限国家）`。
- 代理类型对用户展示为 `SOCKETS`；请求后端仍提交协议码 `2`，并兼容后端短期返回旧展示名 `SOCKS5`。
