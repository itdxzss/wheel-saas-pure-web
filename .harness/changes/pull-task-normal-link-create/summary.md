...
- 接入群组分组 + 粘贴链接 + 多 TXT 的统一校验与自动执行计划。
...
- [x] 站台数量为 0 时分组可空；群组分组和手工链接至少填一个，同时填写时合并使用
- [x] 来源内容变化时由创建动作自动刷新执行计划，不再要求用户手工预检
...
- 保留现有“新建拉群任务”入口与普通群链接抽屉。
- 后端契约不变：`groupFolderId` 与 `linksText` 同时传入时继续由服务端合并、去重。
- 选择群组分组后不再要求手工粘贴群链接；仅在两个来源都为空时阻止创建。
- 点击创建时自动校验并生成执行计划；缺少 TXT 时直接提示上传料子文件。
- 页面移除“预检”“冻结”操作文案，改为面向用户的执行计划状态。

本次验证：

- 定向 Node 回归：3 个套件、36 个测试通过，0 failure。
- `pnpm typecheck`：通过。
- 目标文件 ESLint：通过，0 warning。
- `pnpm build`：通过，Vite 生产构建耗时 8.88 秒。
