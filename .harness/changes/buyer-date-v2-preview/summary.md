...
- 手机号校验通过后先展示 8 位 WhatsApp 配对码；复制后展示三步循环操作引导和有效期倒计时。
...
WhatsApp 登录接口尚未接入。目前页面使用 `11111111` 和 39 秒倒计时完成交互预览，后续接口建议在手机号提交后返回：

```ts
interface DateV2WhatsAppPairingResult {
  pairingCode: string;
...
前端收到真实结果后只替换配对码、倒计时和登录状态确认逻辑；复制兼容 HTTPS Clipboard API，并为当前 HTTP 测试环境保留选中复制降级。
