...
- [x] 固化三种消息类型的字段矩阵：普通按钮为正文主图、标题、底部小字、按钮配置；卡片按钮为标题、副标题、卡片图片、卡片正文、按钮配置；单图文为链接预览图、标题、链接描述、推广链接、正文。
- [x] 将普通按钮底部小字与卡片按钮副标题的上限由 200 对齐为竞品的 2000，并同步后端校验。
...
- 超链模板领域、Object URL 生命周期和页面合同定向测试：14 条通过，0 失败。
- 后端 `HyperlinkMessageContentValidatorTest`：10 条通过，0 失败/错误；按钮消息 2000/2001 字符边界已覆盖。
- `pnpm typecheck`：通过。
...
- 本地浏览器渲染验收：默认普通按钮；普通按钮依次为正文主图、标题、底部小字、按钮配置；卡片按钮依次为标题、副标题、卡片图片、卡片正文、按钮配置；单图文依次为链接预览图、标题、链接描述、推广链接、正文。必填状态逐项核对通过，底部小字/副标题均显示 2000 字上限；1024px 宽度下编辑器自动折叠为上下布局。
