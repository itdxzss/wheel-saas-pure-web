...
import { emptyFeedTaskForm, type FeedTaskForm } from "../constants";
...
function detailToForm(detail: FeedTaskDetail): FeedTaskForm {
  return {
    ...
    startMode: (detail.taskDelayMinutes ?? 0) > 0 ? "scheduled" : "now",
  ...
  async function submitEditor(): Promise<void> {
    ...
    if (
      ...
      (plannedEndTimestamp == null ||
        plannedEndTimestamp <= Date.now() + 60_000)
    ) {
  ...
  async function runAction(
    ...
    try {
      await ElMessageBox.confirm(
        action === "stop"
          ? "停止后任务将终止且无法恢复，是否继续？"
          : `确认${labels[action]}任务「${row.title || row.name}」？`,
        `确认${labels[action]}`,
