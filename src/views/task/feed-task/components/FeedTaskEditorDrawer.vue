...
const hasFilter = computed(
  () => Object.keys(form.value.accountFilter).length > 0
);
      ...
      <section class="feed-preview-panel">
        ...
        >
          <img
            :src="previewImage"
            alt="动态链接预览图"
            class="status-preview-image"
          />
          <div class="status-preview-title">{{ form.title || "推广标题" }}</div>
          <div class="status-preview-description">
            {{ form.description || "点击查看详情" }}
          </div>
          <div class="status-preview-content">
            {{ form.content || "这里是状态正文内容" }}
          </div>
          <div class="status-preview-link">
            {{ form.promotionLink || "https://example.com" }}
          </div>
        </div>
        <el-alert
      ...
      >
        ...
        <el-form-item label="账号范围" required>
          <div class="account-filter-summary">
            <div>
              ...
              <span>{{
                hasFilter
                  ? accountFilterSummary(form.accountFilter)
                  : "未限制（全部有效账号）"
              }}</span>
              <el-tag
                v-loading="accountCountLoading"
                size="small"
                type="success"
                effect="plain"
              >
        ...
        <el-form-item label="链接预览图">
          <div class="image-upload-row">
            ...
            <div>
              ...
              >
                <el-button :disabled="readonly" :icon="useRenderIcon(Upload)"
                  >选择 JPG / JPEG</el-button
                >
              </el-upload>
              <el-button
                v-if="imagePreview"
        ...
        <div class="theme-options">
          <button
            ...
            :class="{
              active:
                form.backgroundColor === theme.backgroundColor &&
                form.textColor === theme.textColor
            }"
            :disabled="readonly"
            :style="{
        ...
        <el-row :gutter="16">
          <el-col :span="12"
            ><el-form-item label="背景颜色"
              ><el-color-picker v-model="form.backgroundColor" /></el-form-item
          ></el-col>
          <el-col :span="12"
            ><el-form-item label="文字颜色"
              ><el-color-picker v-model="form.textColor" /></el-form-item
          ></el-col>
        </el-row>
        ...
        <el-form-item label="任务模式">
          ...
          <div class="field-tip">
            即时任务一轮发完结束；预发布到指定时间结束，期间符合条件的新号自动加入。
          </div>
        </el-form-item>
        <el-form-item
        ...
        >
          <el-date-picker
            v-model="form.taskPlannedEndAt"
            type="datetime"
            value-format="x"
            format="YYYY-MM-DD HH:mm"
            placeholder="选择任务自动结束时间"
            class="w-full"
          />
        </el-form-item>
        <el-row :gutter="16">
        ...
        >
          <el-input-number
            ...
            :step="1"
            class="w-full"
            ><template #suffix>分钟后开始</template></el-input-number
          >
        </el-form-item>
