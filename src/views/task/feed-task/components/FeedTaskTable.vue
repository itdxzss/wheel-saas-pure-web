  ...
  >
    ...
    <template #default="{ dynamicColumns }">
      <el-table v-loading="loading" :data="rows" row-key="id" border>
            ...
            <div class="title-cell">
              ...
              <div class="title-copy">
                <div class="title-line">
                  <span>{{ row.title || "未填写标题" }}</span
                  ><el-tag
                    size="small"
                    effect="plain"
                    :type="row.taskMode === 'rolling' ? 'info' : undefined"
                    >{{
                      row.taskMode === "rolling" ? "预发布" : "即时"
                    }}</el-tag
                  >
                </div>
        ...
        >
          <template #default="{ row }"
            ><el-tag size="small" effect="plain" :type="taskStatusType(row)">{{
              taskStatusLabel(row)
            }}</el-tag></template
          >
        </el-table-column>
            ...
            <div class="progress-cell">
              <div class="progress-label">
                <span class="success-text"
                  >成功 {{ row.successAccountNum ?? 0 }}</span
                ><span class="danger-text"
                  >失败 {{ row.failedAccountNum ?? 0 }}</span
                ><span>共 {{ row.totalAccountNum ?? 0 }}</span>
              </div>
              <el-progress
                :percentage="
                  progress(row.successAccountNum, row.totalAccountNum)
                "
                :show-text="false"
                status="success"
              />
        ...
        >
          <template #default="{ row }">{{
            accountFilterSummary(row.accountFilter)
          }}</template>
        </el-table-column>
        <el-table-column
          v-if="!dynamicColumns[5]?.hide"
          prop="totalAccountNum"
          label="使用号数"
          width="100"
          align="center"
        />
        <el-table-column
          ...
          width="100"
          align="center"
          ><template #default>N</template></el-table-column
        >
        <el-table-column
        ...
        <el-table-column
          ...
          align="center"
          ><template #default="{ row }">{{
            formatFeedTaskTime(row.taskStartAt)
        ...
        <el-table-column
          ...
          align="center"
          ><template #default="{ row }">{{
            formatFeedTaskTime(row.taskPlannedEndAt)
          }}</template></el-table-column
        >
        <el-table-column label="操作" fixed="right" width="270" align="center">
          <template #default="{ row }">
            <el-button
              ...
              :icon="useRenderIcon(actionIcon(action))"
              @click="emit('action', row, action)"
